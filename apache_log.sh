...
#this script parses Apache logs and extracts useful access data
#it must be executed in the directory where access_log is located

echo "The top 10 IPs making the most requests, displaying the IP address and number of requests made:"
cut -f 1 -d - access_log | sort | uniq -c | sort -nr | head -10  

echo "Who is the owner of each of these IPs?"
...
echo "Top 10 requested pages and the number of requests made for each:"
cut -d ' ' -f 7 access_log | sort | uniq -c | sort -nr | head -10

echo "Percentage of successful requests:"
total=$(wc -l < access_log)
ok=$(cut -d ' ' -f 9 access_log | sort | uniq -c | grep 200 | cut -b 5-8)
echo $((($ok * 100) / $total))

echo "Percentage of unsuccessful requests:"
total=$(wc -l < access_log)
fail=$(cut -d ' ' -f 9 access_log | sort | uniq -c | grep 404 | cut -b 4-8)
echo $((($fail * 100) / $total))

echo "The total number of requests made every minute in the time period covered:"
cut -d ' ' -f 4 access_log | cut -d : -f 1-3 | sort | uniq -c

echo "For each of the top 10 IPs, show the top 5 pages requested and the number of requests for each:"
